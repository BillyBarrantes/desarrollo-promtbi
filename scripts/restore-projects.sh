...
fail() {
  jq -cn --arg reason "$1" \
    '{status:"failed", action:"restore_projects", reason:$reason, production_blocked:true}'
  exit 1
}

[[ -n "$project_id" && -n "$task_id" ]] ||
  fail "usage: restore-projects.sh <project_id> <task_id> --dry-run"
[[ "$mode" == "--dry-run" ]] || fail "only --dry-run mode is supported (no real clone)"
command -v jq >/dev/null 2>&1 || fail "jq is required"
[[ -f "$state" ]] || fail "missing state file: $state"

current_branch="$(git branch --show-current)"
[[ "$current_branch" == "$task_id" || "$current_branch" == "agent/$task_id" ]] ||
  fail "restore-projects requires branch agent/$task_id (current=$current_branch)"
...
jq -cn \
  ...
  --arg task_id "$task_id" \
  '{status:"preparation_only", action:"restore_projects",
    mode:"dry_run",
    project_id:$project_id,
    task_id:$task_id,
    repositories_to_restore:$repos,
    destination:"${PROJECTS_ROOT}",
    method:"git_clone",
    note:"no git clone executed in dry-run",
